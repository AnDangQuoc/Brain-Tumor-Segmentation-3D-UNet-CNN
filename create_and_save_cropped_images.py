...
def main():
    ...
    train_val_test_dict['test'] = completelist[0:int(
        len(completelist)*holdout_percentage)]
    trainlist = completelist[int(
        len(completelist)*holdout_percentage):len(completelist)]

    train_percentage = 0.7
    train_val_test_dict['train'] = trainlist[0:int(
        len(trainlist)*train_percentage)]
    train_val_test_dict['val'] = trainlist[int(
        len(trainlist)*train_percentage):len(trainlist)]
    ...
    for i, ID in enumerate(completelist):
        ...
        seg_mask_branch_1 = generate_seg_mask(seg_mask, 1)
        seg_mask_branch_2 = generate_seg_mask(seg_mask, 2)
    ...
    #     y1[i,] = seg_mask_3ch
        ...
        pickle.dump(seg_mask_3ch, open(
            data_dir / f"{ID}_seg_mask_3ch.pkl", "wb"))

        pickle.dump(seg_mask_branch_1, open(
            data_dir / f"{ID}_seg_mask_branch_1.pkl", "wb"))
        pickle.dump(seg_mask_branch_2, open(
            data_dir / f"{ID}_seg_mask_branch_2.pkl", "wb"))
        print("Saving", i+1, "of", len(completelist))


def generate_seg_mask(seg_mask, n_label=2):
    if n_label == 2:
        seg_mask_1 = np.zeros((1, 160, 192, 160))
        seg_mask_2 = np.zeros((1, 160, 192, 160))
        seg_mask_2[seg_mask.astype(int) >= 2] = 1
