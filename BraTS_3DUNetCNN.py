...
from tensorflow.keras.layers import LeakyReLU, Add, UpSampling3D, Activation, SpatialDropout3D
...
from tensorflow.keras import Input, Model
from tensorflow.keras.layers import Conv3D, MaxPooling3D, UpSampling3D, Activation, BatchNormalization, PReLU, Conv3DTranspose, Flatten, Dense, GlobalAveragePooling3D
...
survival_data[survival_data.Brats17ID ==
...
class SingleDataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            y1[i, ] = pickle.load(
                open("./data/%s_seg_mask_3ch.pkl" % (ID), "rb"))
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            y1[i, ] = pickle.load(
                open("./data/%s_seg_mask_3ch.pkl" % (ID), "rb"))
            y2[i, ] = tumor_type_dict[ID]
            y3[i, ] = survival_data[survival_data.Brats17ID ==
                                    ID].Survival.astype(int).values.item(0)
...
# the by_name=True allows you to use a different architecture and bring in the weights from the matching layers
model.load_weights("./weights/1pred_weights.25--0.08.hdf5", by_name=True)
...
cb_2 = keras.callbacks.ModelCheckpoint(filepath="./weights/1Bpred_weights.{epoch:02d}-{val_loss:.2f}.hdf5",
...
cb_1 = keras.callbacks.EarlyStopping(
    monitor='val_loss', min_delta=0, patience=2, verbose=0, mode='auto')
cb_2 = keras.callbacks.ModelCheckpoint(filepath="./weights/2pred_weights.{epoch:02d}-{val_loss:.2f}.hdf5",
                                       monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
model = Model(inputs=inputs, outputs=[
              activation_block, tumortype_block, survival_block])
...
cb_1 = keras.callbacks.EarlyStopping(
    monitor='val_loss', min_delta=0, patience=2, verbose=0, mode='auto')
cb_2 = keras.callbacks.ModelCheckpoint(filepath="./weights/3pred_weights.{epoch:02d}-{val_loss:.2f}.hdf5",
                                       monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='auto', period=1)
...
for i in range(len(prediction)):
    imarray = pickle.load(
        open("./channel_split/prediction_"+str(i)+".pkl", "rb"))
...
#     imsave("./channel_split/"+testIDlist[i]+"ground_truth.tif", ground_truth, 'imagej')
    ...
    imsave("./channel_split/"+testIDlist[i] +
           "ground_truth_1.tif", seg_mask_1, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "ground_truth_2.tif", seg_mask_2, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "ground_truth_3.tif", seg_mask_3, 'imagej')
...
#     imsave("./channel_split/"+testIDlist[i]+"ground_truth.tif", ground_truth, 'imagej')
    ...
    imsave("./channel_split/"+testIDlist[i] +
           "_ground_truth_1.tif", seg_mask_1, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "_ground_truth_2.tif", seg_mask_2, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "_ground_truth_3.tif", seg_mask_3, 'imagej')
    ...
    # switching x and z; c will be taken care of in split
    prediction_thresh = np.rollaxis(prediction_thresh, 1, 3)
    print(np.unique(prediction_thresh))
    print(prediction_thresh.shape)
    ...
    imsave("./channel_split/"+testIDlist[i] +
           "_predicted_1.tif", pred1, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "_predicted_2.tif", pred2, 'imagej')
    imsave("./channel_split/"+testIDlist[i] +
           "_predicted_3.tif", pred3, 'imagej')
